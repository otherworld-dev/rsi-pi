...
class TestCorrectionStatIsNotOnTheWire:
    """POSCORR's Stat cannot be routed back to the PC: the controller rejects
    ETHERNET -> POSCORR -> ETHERNET as 'RSI_CREATE: Circular linking'
    (KR 16-2, 2026-09-10). The shipped Max context therefore has no
    PosCorrStat channel and the API must say so rather than guess."""

    def test_limit_status_is_none_on_max(self, stack):
        api, server = stack
        assert api.monitoring.get_correction_limit_status() is None
        api.motion.update_cartesian(X=1.0)
        _settle()
        assert api.monitoring.get_correction_limit_status() is None
