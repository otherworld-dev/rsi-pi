...
### What the shipped contexts contain

**`RSIPI_Joints`** is the recommended default and the one to read first. It
is KUKA's `RSI_Ethernet` example plus joint corrections, wiring 12 objects
around one `ETHERNET` object (`Timeout=100`,
`ConfigFile=RSI_EthernetConfig_Joints.xml`):

| Direction | Channels | Tags |
|---|---|---|
| SEND (robot → PC) | 1-6 | `DiL` (1, `DIGIN1`), `Digout.o1-o3` (2-4, `DIGOUT1-3`), `Source1` (5, `SOURCE1`), `DoutW` (6, a fourth `DIGOUT` reading back what `MAP2DIGOUT1` writes) |
| RECEIVE (PC → robot) | 1-17 | `RKorr.X-C` (1-6, `POSCORR1`), `FREE` (7, spare), `DiO` (8, `MAP2DIGOUT1`), `AKorr.A1-A6` (9-14, `AXISCORR1`), `SenP1-3` (15-17, `MAP2SEN_PREA1-3`) |

**`RSIPI_Basic`** is the same minus `AXISCORR` and the joint feedback.
**`RSIPI_OnlySend`** is `RSIPI_Basic` with `ONLYSEND` set to `TRUE` in its
config and nothing else changed.

**`RSIPI_Full`** is larger — 16 objects, SEND channels 1-29 and RECEIVE 1-19,
adding `MOTORCURRENT1`, `POSCORRMON1`, `AXISCORRMON1` and `AXISCORREXT1`
(external axes, RECEIVE 13-18). **It is not hardware-verified and its
external-axis objects make the ETHERNET object report `RSIBad` on a robot
without external axes**, so use it only on a cell that has them.

`MAP2SEN_PREA1-3` are wired to their own dedicated channels rather than
sharing the correction channels: sharing meant `$SEN_PREA` values were
rate-limited along with the corrections, and came back as `0.5` instead of
the value sent.

Robot pose, axis positions and motor currents are **not** read through
numbered channels: each `RSI_EthernetConfig_*.xml` declares them as
...
is the `Timeout` parameter of the ETHERNET object — set to 100 in every
shipped context (100 x 4 ms = 0.4 s). KUKA's own default is 10.
...
3. Stop by cancelling the program on the pendant. Stopping the Python side
   ...
   A clean stop signal would require a STOP object in the signal flow
   (manual §7.1.6, p. 42), and **no shipped context includes one —
   deliberately.** Adding a STOP object made the robot silently stop
   applying *all* corrections while RSI carried on running perfectly: no
   error, no log entry, only a pose that never changed. Removing it restored
   motion exactly. Until that can be generated by RSIVisual and diffed
   against a known-good export, `RSI_MOVECORR()` cannot be ended from the
   PC. See [hardware-findings.md](hardware-findings.md).

Keep first corrections small. `POSCORR1` limits the **total** Cartesian
correction, and in relative mode corrections accumulate, so a move simply
stops at the cap with no error anywhere — this is why moves appeared to
stop dead at exactly 5.00 mm under KUKA's ±5 mm example limit. The shipped
contexts raise it to **±50 mm (X/Y/Z) and 45° `MaxRotAngle`**; `AXISCORR1`
in `RSIPI_Joints` allows ±10° per axis. Tighten these in RSIVisual and/or
set software limits on the Python side via `api.safety.set_limit()` before
doing anything real.
