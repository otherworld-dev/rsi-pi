...
| `examples/teleop/` | Drive the robot with an Xbox pad (XInput, nothing to install), the keyboard, or your hand through a webcam (MediaPipe, in a separate disposable env), with a deadman, soft fence and E-stop; record the path and replay it cycle for cycle with a deviation report. Live dashboard. Dry-runs as a scripted square with PASS/FAIL |
